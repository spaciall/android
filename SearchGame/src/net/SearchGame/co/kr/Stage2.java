...
public class Stage2 extends Stage {
	...
	{
		m_StageResultText.Start(150, 200, "Stage2 Start !!", 300, Global.g_StageResultPaint);
		...
		for (int i = 0; i < m_CharMgr.GetSize(); i++) {
			...
			if (chr.GetHP() <= 0) {
				...
				if(m_CharMgr.GetSize() <= 0) {
					// 미션 클리어
					m_StageResultText.Start(150, 200, "Stage2 Clear !!", 300, Global.g_StageResultPaint);
				...
				if (frontChar == chr) {
					...
					ClearStage();
					m_Map.Clear();
					m_StageResultText.Start(150, 200, "Stage2 Failed !!", 300, Global.g_StageResultPaint);
